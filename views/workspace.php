...
<section class="tf-app" data-workspace>
    ...
    <main class="tf-main">
        ...
        <section class="tf-view" data-view="users">
            ...
            <div class="tf-split"><section class="tf-card"><div class="tf-card-head"><div><h2>کاربران</h2><p>اطلاعات، نقش‌ها، وضعیت و رمز عبور را مدیریت کن.</p></div></div><div class="tf-user-list" data-user-list></div></section><section class="tf-card"><div class="tf-card-head"><div><h2>نقش‌های دسترسی</h2><p>کاربران و Permissionهای هر نقش مشخص است.</p></div></div><div class="tf-entity-list tf-role-list" data-role-list></div></section></div>
    ...
    <dialog class="tf-dialog" data-modal="user"><form method="dialog" class="tf-modal-card compact" data-create-user><header><div><h2>حساب کاربری جدید</h2><p>نقش‌های سفارشی و چند نقش هم‌زمان قابل انتخاب‌اند.</p></div><button type="button" data-close-modal>×</button></header><label>نام<input name="name" required></label><label>ایمیل<input name="email" type="email" required></label><label>رمز عبور<input name="password" type="password" minlength="10" required></label><label>نقش‌ها<select name="role_ids" data-options="roles" multiple required></select><small>حداقل یک نقش فعال انتخاب کن.</small></label><footer><button type="button" class="tf-button ghost" data-close-modal>انصراف</button><button class="tf-button">ساخت حساب</button></footer></form></dialog>

    <dialog class="tf-dialog" data-modal="user-edit"><form method="dialog" class="tf-modal-card" data-edit-user><header><div><h2>مدیریت کاربر</h2><p data-user-edit-summary></p></div><button type="button" data-close-modal>×</button></header><input type="hidden" name="user_id"><div class="tf-user-facts"><span><small>آخرین ورود</small><strong data-user-last-login>—</strong></span><span><small>تیم‌ها</small><strong data-user-teams>—</strong></span><span><small>پروژه‌ها</small><strong data-user-projects>—</strong></span><span><small>تسک باز</small><strong data-user-open-tasks>۰</strong></span></div><div class="tf-form-grid"><label>نام<input name="name" required></label><label>ایمیل<input name="email" type="email" required></label><label>وضعیت حساب<select name="status"><option value="active">فعال</option><option value="disabled">غیرفعال</option></select></label><label>نقش‌ها<select name="role_ids" data-options="roles" multiple required></select><small>حذف دسترسی آخرین ادمین کامل مسدود می‌شود.</small></label><label class="span-2">رمز عبور جدید؛ اختیاری<input name="new_password" type="password" minlength="10" autocomplete="new-password"><small>اگر خالی بماند رمز فعلی تغییر نمی‌کند؛ با تغییر رمز نشست‌های قبلی بسته می‌شوند.</small></label></div><footer><button type="button" class="tf-button ghost" data-close-modal>انصراف</button><button class="tf-button">ذخیره تغییرات</button></footer></form></dialog>

    <dialog class="tf-dialog" data-modal="role"><form method="dialog" class="tf-modal-card compact" data-save-role><header><div><h2 data-role-modal-title>تعریف نقش جدید</h2><p data-role-modal-help>دسترسی‌های این نقش را انتخاب کن.</p></div><button type="button" data-close-modal>×</button></header><input type="hidden" name="role_id"><label>کلید انگلیسی نقش<input name="key_name" required placeholder="supervisor"></label><label>نام نمایشی<input name="display_name" required placeholder="سرپرست تولید"></label><label>دسترسی‌ها<select name="permissions" data-options="permissions" multiple></select><small>دسترسی مدیریت نقش‌ها مستقل از مدیریت کاربران است.</small></label><label class="tf-check"><input type="checkbox" name="is_active" checked> نقش فعال باشد</label><footer><button type="button" class="tf-button ghost" data-close-modal>انصراف</button><button class="tf-button">ذخیره نقش</button></footer></form></dialog>
</section>
